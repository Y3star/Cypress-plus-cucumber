...
Then("All fields are valid on login page then screenshot page", () => {
    loginPage.screenshotElement(); // we have error because i have block all my account its normal =)
});

Then("The error message email {string} is displayed on login page", (errorMessageEmail) => { 
    loginPage.elements.errorMessageEmail().should("have.text", errorMessageEmail);
});

Then("The error message password {string} is displayed on login page", (errorMessagePass) => {
    loginPage.elements.errorMessagePass().should("have.text", errorMessagePass);
});